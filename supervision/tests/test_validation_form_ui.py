...
class ValidationFormUiTests(TestCase):
    ...
    def test_validation_page_is_compact_and_decision_oriented(self):
        ...
        self.assertContains(response, 'Ville obligatoire manquante · cas similaire validé')
        self.assertContains(response, 'Téléphone obligatoire manquant · cas similaire validé')
        self.assertNotContains(response, 'Ville obligatoire manquante · 95%')
        self.assertNotContains(response, 'Téléphone obligatoire manquant · 90%')
        self.assertContains(response, 'un pourcentage représente uniquement une probabilité calculée par un modèle de Machine Learning')
